...
class SOM(object):
    ...
    def hit_map(self, data=None,col=np.asarray(plt.cm.RdYlBu(.85))[np.newaxis,:]):
        ...
        # col = np.asarray(plt.cm.RdYlBu(.85))[np.newaxis,:]
        plt.scatter(coord[:, 1], msz[0] - .5 - coord[:, 0], s=area.flatten(), alpha=.9, c=col, marker='o',
                    cmap='jet', linewidths=.1, edgecolor='None')
        plt.xlim(-1, msz[1]+1)
        plt.ylim(-1, msz[0]+1)
        plt.axis('off')
        ...
        if data != None:
            ...
            plt.scatter(coord_d[:, 1] + .5, msz[0] - .5 - coord_d[:, 0], s=area, alpha=0.9, c='None', marker='o',
                        cmap='jet', linewidths=3, edgecolor=col)
            colD = np.asarray(plt.cm.RdYlBu(.15))[np.newaxis,:]
            plt.scatter(coord_d[:, 1] + .5, msz[0] - .5 - coord_d[:, 0], s=area, alpha=0.2, c='r', marker='o',
                        cmap='jet', linewidths=3, edgecolor=colD)
            print ('hi')
...
def view_2d_Pack(self, text_size, which_dim='all', what='codebook', save='No', grid='Yes', save_dir='empty', text='Yes',
    ...
    if what == 'codebook':
        ...
        norm = matplotlib.colors.Normalize(vmin=DD.loc['3%'], vmax=DD.loc['97%'], clip=False)

        while axisNum < dim:
            axisNum += 1

            ax = fig.add_subplot(int(no_row_in_plot), int(no_col_in_plot), int(axisNum))
            ind = int(indtoshow[axisNum - 1])
            mp = codebook[:, ind].reshape(msz0, msz1)
